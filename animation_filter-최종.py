...
class AnimationStylizer:
    def __init__(self):
        ...
        self.realistic_prompts = [
            "photorealistic portrait, young asian person, (detailed facial features:1.3), natural messy curly hair, black t-shirt, red beaded necklace, wireless earbuds, (natural skin texture:1.2), warm lighting, professional photography, dark moody background, high-end fashion portrait, 8k uhd, high detail skin texture",
            "hyperrealistic human portrait, asian youth, (natural face details:1.3), voluminous wavy hair, casual modern fashion, beaded jewelry, earphones, (lifelike skin rendering:1.2), studio lighting setup, dark atmospheric background, fashion magazine quality, detailed skin pores and texture",
            "ultra-detailed human portrait, young asian model, (professional photography:1.3), dynamic curly hair style, contemporary casual wear, bohemian accessories, (photorealistic skin:1.2), cinematic lighting, moody dark backdrop, high fashion, artistically shot, natural facial features"
        ]
    ...
    def process_image(self, image_path, style):
        ...
        try:
            ...
            if style == '1':
                ...
                result = self.apply_realistic_style(image_path)
            else:
    ...
    def apply_realistic_style(self, image_path):
        """사실적 인물 스타일 적용"""
        try:
            # 이미지 전처리
            pil_image = self.preprocess_image(image_path)
            ...
            # 랜덤 프롬프트 선택
            prompt = random.choice(self.realistic_prompts)
            
            # 스타일 변환 (파라미터 최적화)
            result = self.disney_pipe(
                prompt=prompt,
                image=pil_image,
                strength=0.75,        # 원본 특징 유지하면서 사실적 변환
                guidance_scale=7.5,   # 적절한 프롬프트 영향력
                num_inference_steps=50,  # 고품질 렌더링
                negative_prompt="cartoon, anime, illustration, painting, drawing, 3d, digital art, artificial, unnatural skin, smooth skin, plastic skin, oversaturated, deformed features, unrealistic proportions, overexposed"
            ).images[0]
            
            return result
        except Exception as e:
            raise ValueError(f"사실적 스타일 적용 중 오류 발생: {str(e)}")
            ...
            try:
                ...
                if len(faces) > 0:
                    for i, (x, y, w, h) in enumerate(faces):
                        print(f"  얼굴 {i+1}: 위치({x}, {y}), 크기({w}x{h})")
    ...
    def analyze_image(self, image_path):
        ...
        try:
            ...
            try:
                ...
                cv2.imwrite(os.path.join(debug_dir, "gray_processed.jpg"), gray)
                
                # 감지된 얼굴 표시
                debug_image = image.copy()
                for (x, y, w, h) in faces:
                    cv2.rectangle(debug_image, (x, y), (x+w, y+h), (0, 255, 0), 2)
                cv2.imwrite(os.path.join(debug_dir, "detected_faces.jpg"), debug_image)
            except Exception as e:
                print(f"디버그 이미지 저장 중 오류: {str(e)}")
            ...
            print(f"\n=== 이미지 분석 결과 ===")
            print(f"감지된 얼굴 수: {len(faces)}")
            print(f"이미지 채도: {saturation:.2f}")
            print(f"이미지 명도: {value:.2f}")
            print(f"엣지 밀도: {edge_density:.2%}")
            
            # 스타일 추천 로직
            if len(faces) > 0:
                # 얼굴이 있는 경우
                face_size = max([w * h for (x, y, w, h) in faces]) / (512 * 512)
                print(f"가장 큰 얼굴의 크기: {face_size:.2%}")
                
                if saturation > 100 and value > 150:
                    # 밝고 채도가 높은 경우
                    if face_size > 0.2 and edge_density > 0.1:
                        print("선택된 스타일: 디즈니 스타일 (큰 얼굴, 선명한 특징)")
                        return '1'
                    else:
                        print("선택된 스타일: 지브리 스타일 (부드러운 톤)")
                        return '3'
                else:
                    # 자연스러운 톤의 경우
                    if face_size > 0.15 and edge_density > 0.15:
                        print("선택된 스타일: 디즈니 스타일 (현대적인 특징)")
                        return '1'
                    else:
                        print("선택된 스타일: 지브리 스타일 (자연스러운 배경)")
                        return '3'
            else:
                # 얼굴이 없는 경우
                if saturation > 100 and edge_density > 0.2:
                    print("선택된 스타일: 디즈니 스타일 (선명한 특징)")
                    return '1'
                else:
                    print("선택된 스타일: 지브리 스타일 (환상적인 분위기)")
                    return '3'
